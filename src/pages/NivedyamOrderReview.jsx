  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const options = {
        ...
        currency,
        name: "Agraharam",
        description: "Standalone purchase of Nivedyam offerings.",
        order_id: orderId,
        prefill: {
        ...
        handler: async function (response) {
          try {
            const serializedAddress = JSON.stringify({
              isStandaloneNivedyams: true,
              deliveryAddress: address,
              nivedyams: orderData.nivedyams.map(item => ({
                name: item.name,
                quantity: item.quantity,
                description: item.description || "Sacred prasadam offering"
              }))
            });
            ...
            const verifyRes = await apiClient.post(
              `/payment/verify-payment`,
              {
                ...
                pooja_id: null, // Omitted pooja_id for standalone Nivedyam purchase
                total_amount: amount,
            ...
            if (verifyRes.data.success) {
              alert("🎉 Nivedyam purchase successful!");
              navigate("/orders");
            } else {
  ...
  if (!orderData) {
    return (
      <div className="text-center mt-10 text-red-600 font-semibold">
        No cart data found. Please go back and select Nivedyams.
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen bg-[#fffaf0] p-6">
      ...
      <div className="max-w-4xl mx-auto bg-white shadow-md rounded-2xl p-6 border border-orange-100">
        <h2 className="text-2xl font-bold text-orange-800 mb-6">Review Nivedyam Purchase</h2>
          ...
          <ul className="divide-y divide-orange-100">
            {orderData.nivedyams?.map((item, index) => (
              <li key={index} className="py-2.5 flex justify-between text-gray-800 text-sm">
                <span>
                  {item.name} <span className="text-gray-500 text-xs">({item.quantity} {item.unit || "PCS"})</span>
                </span>
        ...
        <div className="mb-6 border-b border-orange-100 pb-4">
          <div className="flex justify-between items-center text-gray-700 mb-1.5 text-sm">
            <span>Nivedyam Cost</span>
            <span>₹{orderData.nivedyamCost}</span>
          </div>
...
export default NivedyamOrderReview;
